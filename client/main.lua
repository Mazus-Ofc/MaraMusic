...
Citizen.CreateThread(function()
	-- while ESX == nil do
	-- 	TriggerEvent('esx:getSharedObject555555555555555', function(obj) ESX = obj end)
	-- 	Citizen.Wait(10)
	-- end
	-- Wait(500)
	...
	TriggerServerEvent("MaraMusic:GetDate")
end)

-- RegisterNetEvent('esx:setJob555555555555555')
-- AddEventHandler('esx:setJob555555555555555', function(job)
-- 	myjob = job
-- end)
...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "play" then
		if xSound:soundExists(nameid) then
			if xSound:isPaused(nameid) then
				TriggerServerEvent("MaraMusic:ChangeState", true, nameid)
				local esperar = 0
				while nuiaberto do
					Wait(1000)
	...
	elseif data.action == "pause" then
		if xSound:soundExists(nameid) then
			if xSound:isPlaying(nameid) then
				TriggerServerEvent("MaraMusic:ChangeState", false, nameid)
			end
		end
	...
	elseif data.action == "loop" then
		if xSound:soundExists(nameid) then
			...
			TriggerServerEvent("MaraMusic:ChangeLoop",nameid,datasoundinfo.loop)
		else
	...
	elseif data.action == "forward" then
		if xSound:soundExists(nameid) then
			TriggerServerEvent("MaraMusic:ChangePosition", 10, nameid)
		end
...
function ApplySound(quanti,plate)
	...
	if vadi <= 1.01 and vadi >= -0.001 and exis then
		...
		TriggerServerEvent("MaraMusic:ChangeVolume", quanti, plate)
	end
end
...
function SetUrl(url,nid)
	...
	if url then
		...
		if encontrad then
			...
			TriggerServerEvent("MaraMusic:ModifyURL",vehdata)
		else
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				local veh = GetVehiclePedIsIn(PlayerPedId(),false)
				...
				vehdata.loop = datasoundinfo.loop
				TriggerServerEvent("MaraMusic:AddVehicle",vehdata)
			end
		end
	else
...
-- if Config.ItemInVehicle then
	RegisterNetEvent("MaraMusic:ShowNui")
	AddEventHandler("MaraMusic:ShowNui", function()
		show()
	end)
-- end
...
function show(nomecenas)
    ...
    elseif nuiaberto then
		...
		-- ESX.ShowNotification("~r~Você não pode fazer isso agora")
	end
end
...
AddEventHandler("MaraMusic:AddVehicle", function(data)
...
AddEventHandler("MaraMusic:ChangeState", function(tipo, nome)
...
AddEventHandler("MaraMusic:ChangeLoop", function(tipo, nome)
	if xSound:soundExists(nome) then
...
AddEventHandler("MaraMusic:ChangeVolume", function(som, range, nome)
	local carroe
	local crds
    for i = 1, #Zones do
        local v = Zones[i]
...
RegisterNetEvent("MaraMusic:SendData")
AddEventHandler("MaraMusic:SendData", function(data)
    Zones = data
    for i = 1, #Zones do
		local v = Zones[i]
		if v.isplaying then
...
Citizen.CreateThread(function()
	while true do
		...
		for i = 1, #Config.Zones do
			...
			if distance <= 10 then
				dormir = 4
				DrawMarker(27,v.changemusicblip.x, v.changemusicblip.y, v.changemusicblip.z-0.98,0,0,0,0.0,0,0,0.8,0.8,0.4,0,255,0,500,0,0,0,1)
				if distance <= 3 then
					DrawText3D(v.changemusicblip.x, v.changemusicblip.y, v.changemusicblip.z, "~r~E~w~ - Carregar Musica")
					if IsControlJustReleased(0, 38) then
						if vSERVER.ChecPermissao(v) then
							nomidaberto = v.name
							show(v.name)
							Wait(1000)
						end
